...
const NEW_ITEMS = [
  {
    id: "arbitrage-cap-fixes-converging-exploit",
    category: "Security",
    text: "A flat per-call percentage cap on arbitrage deployment isn't a cap at all — each call shrinks the balance the next one is measured against, so repeated calls converge on the entire pool. An earlier design was moved past 99% of collateral this way in 40 calls, leaving emergencyWithdraw unable to pay out. The cap here is cumulative against a fixed ceiling that doesn't shrink as it's consumed.\n\n#ArbiSmart #Polygon #USDT #Security",
  },
  {
    id: "swap-router-allowance-cleared-after-use",
    category: "Security",
    text: "Every time pool collateral is approved for a swap, that approval is reset to zero again immediately after the call completes. The router never holds a standing claim on pool funds between transactions — only ever the exact amount of the swap in front of it, for as long as that swap takes.\n\n#ArbiSmart #Polygon #USDT #Security",
  },
];
...
const SCRATCH = `${tmpdir()}/telegram-card-render-batch20-${Date.now()}`;
