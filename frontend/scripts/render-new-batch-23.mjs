...
const NEW_ITEMS = [
  {
    id: "erc1155-hooks-not-optional",
    category: "Security",
    text: "This contract implements the ERC-1155 receiver hooks Polymarket's own token contract calls back to on every split — not decoration, a hard requirement. Without them, the mint's acceptance check fails and every split reverts, in every market, permanently. Deliberately unrestricted to accept any token sent: refusing would risk blocking a legitimate mint for no real security gain.\n\n#ArbiSmart #Polygon #USDT #Security",
  },
  {
    id: "per-user-deposit-breakdown-view",
    category: "Transparency",
    text: "Anyone can look up any address's own numbers in a single call: total gross deposited, platform fees paid, net amount actually staked, and current active stake. Same read-only, on-chain transparency as the pool-wide dashboard figures — just scoped down to one specific staker instead of the whole pool.\n\n#ArbiSmart #Polygon #USDT #Transparency",
  },
];
...
const SCRATCH = `${tmpdir()}/telegram-card-render-batch23-${Date.now()}`;
