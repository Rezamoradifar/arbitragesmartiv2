...
const NEW_ITEMS = [
  {
    id: "deposit-verified-by-balance-delta",
    category: "Security",
    text: "Every deposit into this contract — a stake, a top-up, an owner-granted stake — is verified by comparing the contract's own token balance before and after the transfer, not by trusting the amount requested. A fee-on-transfer or otherwise non-standard token that delivers less than expected gets caught immediately; the position is never credited for collateral that didn't actually arrive.\n\n#ArbiSmart #Polygon #USDT #Security",
  },
  {
    id: "exit-is-permanent-per-address",
    category: "Transparency",
    text: "Once an address exits a position — early or through the emergency escape hatch — that flag is permanent: the same address can never open a new stake in this contract again, ever. It's a deliberate one-way door, not a bug; a wallet that's already been paid out can't cycle back in.\n\n#ArbiSmart #Polygon #USDT #Transparency",
  },
];
...
const SCRATCH = `${tmpdir()}/telegram-card-render-batch40-${Date.now()}`;
