...
const NEW_ITEMS = [
  {
    id: "profit-fee-only-on-real-gains",
    category: "Transparency",
    text: "When arbitrage capital swaps back to collateral, the contract retires its own tracked cost basis first — only what's left above that gets counted as profit. The performance fee is calculated on that leftover alone, so returned principal is mathematically never taxed, not just described that way.\n\n#ArbiSmart #Polygon #USDT #Transparency",
  },
  {
    id: "per-condition-commitment-tracked",
    category: "Transparency",
    text: "Capital committed to each Polymarket market is tracked separately, keyed to that specific market's condition. When a position redeems, the contract knows exactly how much of that particular market's principal it's getting back — commitments across different open positions are never lumped together or guessed at.\n\n#ArbiSmart #Polygon #USDT #Transparency",
  },
];
...
const SCRATCH = `${tmpdir()}/telegram-card-render-batch19-${Date.now()}`;
