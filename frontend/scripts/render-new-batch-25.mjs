...
const NEW_ITEMS = [
  {
    id: "three-revenue-streams-separately-tracked",
    category: "Transparency",
    text: "Three separate kinds of platform revenue — the deposit fee, the yield-claim fee, and the arbitrage performance fee — each go to their own distinct wallet address, tracked independently on-chain. Nothing is pooled together into one number; every revenue stream stays separately attributable to exactly where it came from.\n\n#ArbiSmart #Polygon #USDT #Transparency",
  },
  {
    id: "removepartner-clears-live-vote",
    category: "Security",
    text: "Removing a partner automatically clears any emergency vote they'd already cast and drops the tally by one — the vote count can never silently drift out of sync with who's actually still in the voting body. This can only happen outside emergency mode in the first place, since the voting body itself is frozen once a vote is live.\n\n#ArbiSmart #Polygon #USDT #Security",
  },
];
...
const SCRATCH = `${tmpdir()}/telegram-card-render-batch25-${Date.now()}`;
