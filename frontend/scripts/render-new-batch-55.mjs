...
const NEW_ITEMS = [
  {
    id: "emergency-voting-closes-once-quorum-hits",
    category: "Security",
    text: "The moment an emergency vote reaches quorum, casting a NEW emergency vote becomes uncallable — the function itself is blocked once emergency mode is active. A partner who hadn't voted yet can't add their voice after the fact; only revoking an already-cast vote stays open, and only until the delay makes even that irrevocable.\n\n#ArbiSmart #Polygon #USDT #Security",
  },
  {
    id: "rescue-voting-stays-open-through-emergency-mode",
    category: "Security",
    text: "Unlike an emergency vote, which stops being castable the instant quorum is reached, a rescue vote has no such lockout — a partner can still cast one even after emergency mode is already active. The two votes are deliberately independent: arming a fund rescue can keep gathering support through and after the point the withdrawal escape hatch has already opened.\n\n#ArbiSmart #Polygon #USDT #Security",
  },
];
...
const SCRATCH = `${tmpdir()}/telegram-card-render-batch55-${Date.now()}`;
