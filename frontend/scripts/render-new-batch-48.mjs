...
const NEW_ITEMS = [
  {
    id: "rescue-vote-revocable-anytime-unlike-emergency",
    category: "Security",
    text: "A rescue vote can be pulled back at any time right up until the sweep actually executes — even after the multi-day delay has fully elapsed. That's different from an emergency vote, which locks in and can no longer be revoked once the withdrawal window opens. A single partner changing their mind at the last second still stops the sweep, because the contract re-checks quorum at execution time, not just at the moment the vote first passed.\n\n#ArbiSmart #Polygon #USDT #Security",
  },
  {
    id: "arbitrage-profit-deposit-is-one-way",
    category: "Transparency",
    text: "The function that credits externally-earned arbitrage profit back into the pool only moves value in — there's no matching function that lets the owner pull it back out to a wallet. Once profit is deposited this way, it becomes pool capital like any other, not a balance the owner can later reclaim for themselves.\n\n#ArbiSmart #Polygon #USDT #Transparency",
  },
];
...
const SCRATCH = `${tmpdir()}/telegram-card-render-batch48-${Date.now()}`;
