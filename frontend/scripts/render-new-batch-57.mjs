...
const NEW_ITEMS = [
  {
    id: "plan-upgrade-can-shorten-earning-window",
    category: "Staking",
    text: "Higher plans pay more per day but run for fewer days — Starter earns for 180 days, Elite for only 90. Your cutoff date is your ORIGINAL stake's start time plus whatever plan you currently hold, so upgrading to a higher tier can pull that cutoff earlier, not later, even though your start date never changes.\n\n#ArbiSmart #Polygon #USDT #Staking",
  },
  {
    id: "active-stake-count-tracked-but-never-exposed",
    category: "Transparency",
    text: "The contract keeps a running internal counter of currently active stakers — incremented on every new stake, decremented on every exit — but never actually exposes it through any view function. What getGlobalStats() reports instead is a different number entirely: the lifetime count of every address that has ever staked, which only ever goes up.\n\n#ArbiSmart #Polygon #USDT #Transparency",
  },
];
...
const SCRATCH = `${tmpdir()}/telegram-card-render-batch57-${Date.now()}`;
