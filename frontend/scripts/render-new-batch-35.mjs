...
const NEW_ITEMS = [
  {
    id: "adding-partners-blocked-during-emergency",
    category: "Security",
    text: "The owner cannot add a new partner to the voting body while an emergency vote is already live. Without that block, an owner facing a real vote could pack the body with friendly addresses to dilute it out mid-flight — blocking the addition during emergency mode closes that door entirely.\n\n#ArbiSmart #Polygon #USDT #Security",
  },
  {
    id: "emergency-vote-quorum-triggers-immediate-pause",
    category: "Transparency",
    text: "The moment an emergency vote reaches quorum, three things happen in the same transaction: the contract pauses immediately, new arbitrage deployments stop cold, and a fixed delay clock starts ticking toward the point where every staker can pull their own principal out through the emergency exit — no further action from the owner required, and none possible to stop it.\n\n#ArbiSmart #Polygon #USDT #Transparency",
  },
];
...
const SCRATCH = `${tmpdir()}/telegram-card-render-batch35-${Date.now()}`;
