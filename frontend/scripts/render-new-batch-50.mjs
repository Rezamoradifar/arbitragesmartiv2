...
const NEW_ITEMS = [
  {
    id: "referral-payout-capped-by-running-remainder",
    category: "Referral",
    text: "Referral shares for the three upline levels are paid out of a running remainder, not calculated independently and added up afterward. Each level's share is capped against whatever budget is left after the levels before it were paid, so three mismatched tier rates can never sum to more than the claim itself — it's impossible by construction, not just unlikely in practice.\n\n#ArbiSmart #Polygon #USDT #Referral",
  },
  {
    id: "blacklisted-referrer-share-stays-in-pool",
    category: "Referral",
    text: "If someone in your three-level upline chain is blacklisted, their share of your claim isn't rerouted to the next person up the chain — it simply stays in the pool. Blacklisting one link doesn't let the referrer above them collect an extra cut; that share is skipped entirely, not redirected.\n\n#ArbiSmart #Polygon #USDT #Referral",
  },
];
...
const SCRATCH = `${tmpdir()}/telegram-card-render-batch50-${Date.now()}`;
